...
def command(cmd): #Ôóíêöèÿ, êîòîðàÿ âûïîëíÿåò SQL-çàïðîñ, è âîçâðàùàåò ñòðîêè èç òàáëèöû
...
def parent_table(table, value, operation, item = None, item_2 = None): #Ôóíêöèÿ âûïîëíÿþùàÿ çàïðîñû äëÿ ðîäèòåëüñêèõ òàáëèö
    result = ""
    match operation:
        case 'INSERT':
...
#Ôóíêöèÿ âûïîëíÿþùàÿ çàïðîñû äëÿ òàáëèöû main
def main(operation,
        fam = "Not stated",
        name = "Not stated", 
        ...
        appr = "Not stated", 
        telef = "Not stated",
        new_values = dict()): #new_values ïåðåäà¸ì òîëüêî äëÿ îïåðàöèè UPDATE; ìà
    ...
    match operation:
        case 'INSERT':
            ...
            result = f"""INSERT INTO main ({columns})
                        VALUES ({usable})"""
        case 'UPDATE': #Ïðàâü ýòó ôóíêöèþ
            #print(new_values)
            columns = [columns[i] for i in range(len(usable)) if usable[i] != 'Not stated' and usable[i] != ""] #Âûáèðàåì ñòîëáöû, çíà÷åíèå êîòîðûõ áóäåò îáíîâëÿòüñÿ 
            #print(columns)
            ...
            for i in range(len(usable)):
                if i not in kyes:
                    usable[i] = f"'{usable[i]}'"
            statement = [] #Âûðàæåíèå, êîòîðîå áóäåò ñòîÿòü ïîñëå WHERE
            for i in range(len(usable)):
            ...
            values = [] #Âûðàæåíèå, êîòîðîå áóäåò ñòîÿòü äî WHERE
        ...
        case 'DELETE':
            columns = [columns[i] for i in range(len(usable)) if usable[i] != 'Not stated' and usable[i] != ""] #Âûáèðàåì ñòîëáöû, ïî êîòîðûì áóäåò óäàëåíèå
            usable = [i for i in usable if i != 'Not stated' and i != ""]
            for i in range(len(columns)):
                if columns[i] in integer:
                    kyes.append(i)
        ...
        case 'SELECT': #Îíà æå ôóíêöèÿ ôèëüòðàöèè
            columns = [columns[i] for i in range(len(usable)) if usable[i] != 'Not stated' and usable[i] != ""] #Âûáèðàåì ñòîëáöû äëÿ âûáîðêè
            usable = [i for i in usable if i != 'Not stated' and i != ""]
...
class main_window(QMainWindow):
    ...
    def main_manipulate(self):
        ...
        if query_items[3] != "Not stated":
            ...
            street = "Not stated"
        if operation == "UPDATE":
            #Íå ïðåäóñìîòðåíî çíà÷åíèå Not stated
            ...
            new_data["bldng"] = self.second_window.bldng_lineEdit_2.text() #Äîáàâü äàííûå äëÿ îáíîâëåíèÿ
        ...
        try:
            ...
            if operation == "SELECT":
                ...
                self.table_buffer = get_table(self.table_buffer_queary) # Ñòîèò íå ïîëó÷àòü çàíîâî âñþ òàáëèöó, à äîáàâëÿòü íîâóþ çàïèñü
